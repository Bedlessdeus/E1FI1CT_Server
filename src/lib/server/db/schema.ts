...
export const userActivity = sqliteTable('user_activity', {
	...
	userId: text('user_id')
		.notNull()
		.references(() => user.id, { onDelete: 'cascade' }),
	activityType: text('activity_type').notNull(), 	targetId: text('target_id'), 	metadata: text('metadata'), 	createdAt: integer('created_at', { mode: 'timestamp' }).notNull()
}, (table) => ({
...
export const follow = sqliteTable('follow', {
	...
	followerFollowingIdx: uniqueIndex('follow_follower_following_idx').on(table.followerId, table.followingId),
